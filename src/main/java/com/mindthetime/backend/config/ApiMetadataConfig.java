    ...
    public OpenAPI stationlyOpenAPI() {
        return new OpenAPI()
                .info(new Info()
                        ...
                        .version("v1.0.0")
                        .contact(new Contact().name("Stationly Limited").email("support@stationly.co.uk"))
                        .license(new License().name("Apache 2.0").url("http://springdoc.org")))
                .servers(List.of(
                        new Server().url("https://api.stationly.co.uk/MindTheTimeBE").description("Production Server"),
                        new Server().url("http://localhost:8080/MindTheTimeBE").description("Local Development")));
    }
}